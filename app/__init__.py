...
# used for local usage not for supabase.com
# import psycopg2
...
def create_app():
    ...
    if not config_name:
        raise RuntimeError(
            "The FLASK_CONFIG environment variable is not set. Please set it to 'development', 'production', or another valid environment.")

    # Ensure the configuration name is valid
    if config_name not in config_map:
        raise ValueError(
            f"Invalid FLASK_CONFIG value: '{config_name}'. Valid options are: {', '.join(config_map.keys())}")
